...
// For percentages
export function formatPercent(value: number | bigint | string): string {
  // Convert to number if needed
  const numValue = typeof value === 'bigint' ? Number(value)
    : typeof value === 'string' ? parseFloat(value)
    : value;
...
// For token amounts
export function formatTokenAmount(value: number | bigint | string, maxDecimals: number = 6): string {
  // Convert to number if needed
  const numValue = typeof value === 'bigint' ? Number(value)
    : typeof value === 'string' ? parseFloat(value)
    : value;
...
export function formatPrice(value: number | bigint | string): string {
  // Convert to number if needed
  const numValue = typeof value === 'bigint' ? Number(value)
    : typeof value === 'string' ? parseFloat(value)
    : value;
  
  if (!numValue || isNaN(numValue) || numValue === 0) return "$0.00";
...
export function formatPercentage(value: number | bigint | string): string {
  // Convert to number if needed
  const numValue = typeof value === 'bigint' ? Number(value)
    : typeof value === 'string' ? parseFloat(value)
    : value;
